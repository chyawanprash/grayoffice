...
/* ── the canvas ── */
export function Flowchart({ steps = NODES }: { steps?: StepNode[]; variant?: string } = {}) {
      ...
      <svg width={cw} height={canvasH} className="pointer-events-none absolute inset-0">
        {EDGES.map((edge) => (
          <path
            ...
            d={bezier(edge)}
            fill="none"
            stroke={isLit(edge) ? "var(--color-aic)" : "var(--line-strong)"}
            strokeWidth="1.25"
          ...
          >
            {node.kind && (
              <span
                ...
                style={{
                  background: mix(node.kind.hue, 14, "var(--color-page)"),
            ...
            {node.condition ? (
              ...
              <button
                ...
                className={`w-full cursor-pointer rounded-[18px] bg-surface text-left outline-none
                  transition-shadow duration-150 focus-visible:shadow-[0_0_0_1.5px_var(--color-aic)]
                  ${
                    active
                      ? "shadow-[0_0_0_1.5px_var(--color-aic),0_2px_10px_rgba(0,0,0,0.045)]"
